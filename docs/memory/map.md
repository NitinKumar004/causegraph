...
- Web UI (M6, read-only) → `engine/causegraph/api/server.py`: pure `graph_payload(db,pid|q,min_confidence,max_nodes)` serializes the culprit neighborhood (bounded on both axes, `truncated` flag) + `ThreadingHTTPServer` bound to 127.0.0.1 (db fixed at startup, allowlist static routes). Page: `ui/index.html`+`app.js`+vendored `ui/vendor/cytoscape.min.js`. Launch `cg ui`. BFS walk = `traverse.descendants_bfs`. confidence: high
