...
| **M6** — web UI | read-only local viewer: `cg ui` serves a page (vendored cytoscape.js, offline) that renders the causal graph for a pid — process vs file nodes, edges coloured by rule+confidence, payload bounded (`max_nodes`); stdlib http.server API bound to 127.0.0.1. Live auto-refresh + live/frozen indicator; monochrome UI; responsive | [plans/causegraph-M6.md](plans/causegraph-M6.md) |
| **M7** — one-command use | `cg up` runs a **detached background recorder** (survives the shell, state in `~/.causegraph/`) and serves the live UI; `cg down` / `cg status`. Makes it usable by another person without babysitting a terminal | [plans/causegraph-M7-one-command.md](plans/causegraph-M7-one-command.md) |
